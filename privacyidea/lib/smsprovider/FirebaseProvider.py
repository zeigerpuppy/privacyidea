...
def get_firebase_access_token(config_file_name):
    ...
    if not isinstance(app_store[fbt].get(config_file_name), Credentials) or \
            datetime.datetime.now() > app_store[fbt].get(config_file_name).expiry:
        # If the type of the config is not class AccessToken or
        # if the token has expired
        log.debug("Fetching Credentials for firebase from {!r}...".format(config_file_name))
        credentials = Credentials.from_service_account_file(config_file_name, scopes=SCOPES)
        # We do not use a lock here: The worst that could happen is that two threads
        # fetch new auth tokens concurrently. In this case, one of them wins
        # and is written to the dictionary.
        app_store[fbt][config_file_name] = credentials
        log.debug(u"Setting the expiration for {!r} of the new access_token "
                  u"to {!s}.".format(config_file_name, credentials.expiry.isoformat()))
...
class FirebaseProvider(ISMSProvider):
    ...
    def submit_message(self, firebase_token, data):
        ...
        resp = authed_session.post(url, data=json.dumps(fcm_message),
